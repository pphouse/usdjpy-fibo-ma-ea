...
class M15BestVisualizer:
    def __init__(self):
        self.tp_pips = 40
        self.sl_pips = 40
        self.nanpin_interval = 5
    ...
    def load_data(self):
        ...
        df_m1 = load_m1_data()
        print("M15に変換中...")
        self.df = df_m1.resample('15min').agg({
    ...
    def visualize_full_performance(self, save_path):
        ...
        fig.suptitle(f'M15 Best Parameters: TP{self.tp_pips}/SL{self.sl_pips}/Nanpin{self.nanpin_interval}\n'
        ...
        nanpin_perf = df_trades.groupby('nanpin_count').agg({'weighted_pips': ['sum', 'count']})
        nanpin_perf.columns = ['total_pips', 'count']

        x = np.arange(len(nanpin_perf))
        width = 0.35
        ax3.bar(x - width/2, nanpin_perf['total_pips'], width, label='Total Pips', alpha=0.7)
        ax3_twin = ax3.twinx()
        ax3_twin.bar(x + width/2, nanpin_perf['count'], width, label='Count', color='orange', alpha=0.7)
        ax3.set_xticks(x)
        ...
        ax4.set_title(f'Drawdown Analysis (Max DD: {max_dd:,.0f})', fontsize=12, fontweight='bold')
...
"""
    ...
    def visualize_monthly(self, save_path):
        ...
        fig.suptitle('Monthly Performance Analysis (M15)', fontsize=14, fontweight='bold')
...
def main():
    print("="*70)
    print("  M15ベストパラメータ 詳細可視化")
    print("  TP40/SL40/Nanpin5 (N3固定)")
    print("="*70)

    viz = M15BestVisualizer()
    viz.load_data()
    viz.run_backtest()

    print("\n可視化を生成中...")
    viz.visualize_full_performance('/Users/naoto/ドル円/m15_real_backtest/best_performance.png')
    viz.visualize_monthly('/Users/naoto/ドル円/m15_real_backtest/best_monthly.png')
