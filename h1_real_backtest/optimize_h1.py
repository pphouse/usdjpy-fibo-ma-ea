...
class H1Optimizer:
    ...
    def load_data(self):
        ...
        print(f"H1データ: {len(self.df):,}本 ({self.df.index[0].date()} ~ {self.df.index[-1].date()})")

        # CSV保存
        output_path = '/Users/naoto/ドル円/h1_real_backtest/USDJPY_H1_2015_2025.csv'
    ...
    def run_single_backtest(self, tp_pips, sl_pips, nanpin_interval, tolerance_pips=25):
        """単一パラメータでバックテスト"""
        lot_ratios = [1, 3, 3, 5]
        max_nanpin = 3
        pip_value = 0.01
    ...
    def run_optimization(self):
        ...
        # パラメータ範囲（H1用に調整）
        tp_range = [30, 50, 70, 80, 100, 120]
        sl_range = [30, 50, 70, 80, 100, 120]
        nanpin_range = [10, 15, 20, 25, 30, 40]
    ...
    def visualize_optimization(self, save_path):
        ...
        fig.suptitle('H1 Real Data Parameter Optimization (N3 Fixed, Axiory 2015-2025)',
    ...
    def visualize_heatmaps(self, save_path):
        ...
        fig.suptitle('Parameter Heatmaps (H1 Real Data, N3 Fixed)', fontsize=14, fontweight='bold')
...
def main():
    print("="*70)
    print("  H1実データ パラメータ最適化")
    print("  USDJPY 2015-2025 (Axiory M1 → H1)")
    print("  N3固定 (1:3:3:5)")
    print("="*70)

    opt = H1Optimizer(spread_pips=0.3)
    opt.run_optimization()
    top20 = opt.print_results()

    # 可視化
    opt.visualize_optimization('/Users/naoto/ドル円/h1_real_backtest/optimization_results.png')
    opt.visualize_heatmaps('/Users/naoto/ドル円/h1_real_backtest/optimization_heatmaps.png')
    opt.save_results('/Users/naoto/ドル円/h1_real_backtest/optimization_results.csv')
