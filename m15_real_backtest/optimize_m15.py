...
class M15Optimizer:
    ...
    def load_data(self):
        ...
        print("M15に変換中...")
        self.df = resample_to_m15(df_m1)

        for period in [20, 50, 100, 200]:
            self.df[f'ma_{period}'] = self.df['close'].rolling(window=period).mean()

        print(f"M15データ: {len(self.df):,}本 ({self.df.index[0].date()} ~ {self.df.index[-1].date()})")

        output_path = '/Users/naoto/ドル円/m15_real_backtest/USDJPY_M15_2015_2025.csv'
        self.df.to_csv(output_path)
        print(f"保存: {output_path}")

        return self.df

    def calculate_fibonacci(self, idx, lookback=200):
        """フィボナッチレベル計算"""
        if idx < lookback:
            return None, None, 0
    ...
    def run_single_backtest(self, tp_pips, sl_pips, nanpin_interval, tolerance_pips=15):
        """単一パラメータでバックテスト"""
        lot_ratios = [1, 3, 3, 5]
        max_nanpin = 3
        pip_value = 0.01
        lookback = 200  # M15用に調整
        ...
        for idx in range(lookback + 200, len(self.df) - 500, step):
            ...
            for i in range(idx + 1, min(idx + 4000, len(self.df))):
    ...
    def run_optimization(self):
        ...
        # パラメータ範囲（M15用に調整）
        tp_range = [15, 20, 30, 40, 50, 60]
        sl_range = [15, 20, 30, 40, 50, 60]
        nanpin_range = [5, 8, 10, 15, 20, 25]
    ...
    def visualize_optimization(self, save_path):
        ...
        fig.suptitle('M15 Real Data Parameter Optimization (N3 Fixed, Axiory 2015-2025)',
    ...
    def visualize_heatmaps(self, save_path):
        ...
        fig.suptitle('Parameter Heatmaps (M15 Real Data, N3 Fixed)', fontsize=14, fontweight='bold')
...
def main():
    print("="*70)
    print("  M15実データ パラメータ最適化")
    print("  USDJPY 2015-2025 (Axiory M1 → M15)")
    print("  N3固定 (1:3:3:5)")
    print("="*70)

    opt = M15Optimizer(spread_pips=0.3)
    opt.run_optimization()
    top20 = opt.print_results()

    opt.visualize_optimization('/Users/naoto/ドル円/m15_real_backtest/optimization_results.png')
    opt.visualize_heatmaps('/Users/naoto/ドル円/m15_real_backtest/optimization_heatmaps.png')
    opt.save_results('/Users/naoto/ドル円/m15_real_backtest/optimization_results.csv')
