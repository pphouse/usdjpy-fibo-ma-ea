...
class H4Optimizer:
    ...
    def load_data(self):
        ...
        print(f"データ: {len(self.df):,}本 ({self.df.index[0].date()} ~ {self.df.index[-1].date()})")
        return self.df
    ...
    def run_single_backtest(self, tp_pips, sl_pips, nanpin_interval, tolerance_pips=35):
        """単一パラメータでバックテスト"""
        lot_ratios = [1, 3, 3, 5]
        max_nanpin = 3
        pip_value = 0.01
    ...
    def run_optimization(self):
        ...
        # パラメータ範囲
        tp_range = [50, 70, 80, 100, 120, 150]
        sl_range = [50, 70, 80, 100, 120, 150]
        nanpin_range = [15, 20, 25, 30, 40, 50]
    ...
    def get_top_results(self, n=20, sort_by='total_pips'):
        """上位N件を取得"""
    ...
    def visualize_optimization(self, save_path):
        ...
        fig.suptitle('H4 Real Data Parameter Optimization (N3 Fixed, Axiory 2015-2025)',
                    fontsize=16, fontweight='bold')

        # 1. TP vs SL ヒートマップ (Net Pips)
        ...
        for i, (_, row) in enumerate(top10.iterrows(), 1):
            profit = row['total_pips'] * 100  # 0.1lot
            table_data.append([
                f"#{i}",
                f"{int(row['tp'])}",
                f"{int(row['sl'])}",
    ...
    def visualize_heatmaps(self, save_path):
        ...
        fig.suptitle('Parameter Heatmaps (H4 Real Data, N3 Fixed)', fontsize=14, fontweight='bold')
...
def main():
    print("="*70)
    print("  H4実データ パラメータ最適化")
    print("  USDJPY 2015-2025 (Axiory)")
    print("  N3固定 (1:3:3:5)")
    print("="*70)

    opt = H4Optimizer(spread_pips=0.3)
    opt.run_optimization()
    top20 = opt.print_results()

    # 可視化
    opt.visualize_optimization('/Users/naoto/ドル円/h4_real_backtest/optimization_results.png')
    opt.visualize_heatmaps('/Users/naoto/ドル円/h4_real_backtest/optimization_heatmaps.png')
    opt.save_results('/Users/naoto/ドル円/h4_real_backtest/optimization_results.csv')
