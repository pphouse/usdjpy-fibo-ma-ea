...
def resample_to_m30(df_m1):
    """M1をM30に変換"""
    df_m30 = df_m1.resample('30min').agg({
        'open': 'first',
        'high': 'max',
        'low': 'min',
        'close': 'last',
...
class M30Optimizer:
    ...
    def load_data(self):
        ...
        print("M30に変換中...")
        self.df = resample_to_m30(df_m1)

        for period in [20, 50, 100, 200]:
            self.df[f'ma_{period}'] = self.df['close'].rolling(window=period).mean()

        print(f"M30データ: {len(self.df):,}本 ({self.df.index[0].date()} ~ {self.df.index[-1].date()})")

        output_path = '/Users/naoto/ドル円/m30_real_backtest/USDJPY_M30_2015_2025.csv'
        self.df.to_csv(output_path)
        print(f"保存: {output_path}")

        return self.df

    def calculate_fibonacci(self, idx, lookback=150):
        """フィボナッチレベル計算"""
        if idx < lookback:
            return None, None, 0
    ...
    def run_single_backtest(self, tp_pips, sl_pips, nanpin_interval, tolerance_pips=20):
        """単一パラメータでバックテスト"""
        lot_ratios = [1, 3, 3, 5]
        max_nanpin = 3
        pip_value = 0.01
        lookback = 150  # M30用に調整
        ...
        for idx in range(lookback + 200, len(self.df) - 300, step):
    ...
    def run_optimization(self):
        ...
        # パラメータ範囲（M30用に調整）
        tp_range = [20, 30, 40, 50, 70, 80]
        sl_range = [20, 30, 40, 50, 70, 80]
        nanpin_range = [8, 10, 15, 20, 25, 30]
    ...
    def visualize_optimization(self, save_path):
        ...
        fig.suptitle('M30 Real Data Parameter Optimization (N3 Fixed, Axiory 2015-2025)',
    ...
    def visualize_heatmaps(self, save_path):
        ...
        fig.suptitle('Parameter Heatmaps (M30 Real Data, N3 Fixed)', fontsize=14, fontweight='bold')
...
def main():
    print("="*70)
    print("  M30実データ パラメータ最適化")
    print("  USDJPY 2015-2025 (Axiory M1 → M30)")
    print("  N3固定 (1:3:3:5)")
    print("="*70)

    opt = M30Optimizer(spread_pips=0.3)
    opt.run_optimization()
    top20 = opt.print_results()

    opt.visualize_optimization('/Users/naoto/ドル円/m30_real_backtest/optimization_results.png')
    opt.visualize_heatmaps('/Users/naoto/ドル円/m30_real_backtest/optimization_heatmaps.png')
    opt.save_results('/Users/naoto/ドル円/m30_real_backtest/optimization_results.csv')
