...
class M5BestVisualizer:
    def __init__(self):
        self.tp_pips = 40
        self.sl_pips = 40
        self.nanpin_interval = 3
    ...
    def load_data(self):
        ...
        df_m1 = load_m1_data()
        print("M5に変換中...")
        self.df = df_m1.resample('5min').agg({
    ...
    def visualize_full_performance(self, save_path):
        ...
        fig.suptitle(f'M5 Best Parameters: TP{self.tp_pips}/SL{self.sl_pips}/Nanpin{self.nanpin_interval}\n'
                    f'Axiory 2015-2025 Real Data (GogoJungle Most Popular Timeframe)',
        ...
        summary_text = f"""
═══════════════════════════════════════════
           PERFORMANCE SUMMARY (M5)
          GogoJungle Most Popular TF
═══════════════════════════════════════════
...
"""
    ...
    def visualize_monthly(self, save_path):
        ...
        fig.suptitle('Monthly Performance Analysis (M5 - GogoJungle Most Popular)', fontsize=14, fontweight='bold')
...
def main():
    print("="*70)
    print("  M5ベストパラメータ 詳細可視化")
    print("  TP40/SL40/Nanpin3 (N3固定)")
    print("  ※ゴゴジャン人気No.1の時間枠")
    print("="*70)

    viz = M5BestVisualizer()
    viz.load_data()
    viz.run_backtest()

    print("\n可視化を生成中...")
    viz.visualize_full_performance('/Users/naoto/ドル円/m5_real_backtest/best_performance.png')
    viz.visualize_monthly('/Users/naoto/ドル円/m5_real_backtest/best_monthly.png')
